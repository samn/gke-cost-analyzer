...
### Changed
- Upgraded the Go toolchain to 1.26.4.toml`, `mise.lock`, and the `go.mod` `go` directive).
...
### Added
- **Node overhead is now tracked as waste for standard GKE workloads**: unallocated node capacity cost is attributed proportionally to running pods and always counted toward `WASTE`. Waste is the sum of two disjoint parts — node overhead (always) plus the requested-but-unused portion of each group's own resource cost (when Prometheus utilization data is available). A fully utilized workload on an underpacked node still reports its node overhead as waste. Standard-mode `WASTE` is shown in `watch` and `history` even without Prometheus data. Always zero for Autopilot pods (whose waste is unchanged).
